...
''' Trabajo Practico Final: NAPO'S BAKERY '''
...
# class Torta:
#
#     def __init__(self, sabor, relleno, tipo, tamanio, tiempopreparacion):
#         self.sabor = sabor
#         self.relleno = relleno
#         self.tipo = tipo
#         self.tamanio = tamanio
#         self.__tiempopreparacion = tiempopreparacion
#
...
#
# class Flores(Torta):
#     def __init__(self, sabor, relleno, tipo, tamanio, tiempopreparacion, decoracion):
#         super().__init__(sabor, relleno, tipo, tiempopreparacion, tamanio)
...
#
# instancia_torta = Torta(sabor, relleno, tipo, tamanio, tiempopreparacion)
# instancia_torta.calcular_precio()
# instancia_torta.__calculartiempoprep()
# print(instancia_torta.__calculartiempoprep())
#
# instancia_flores = Flores(sabor, relleno, tipo, tamanio, tiempopreparacion, decoracion)
# instancia_flores.calcular_precio()

''' flores hereda los atributos(herenica), y le suma decoracion. el metodo para calcular el precio, sale 50% mas si decora con flores(polimofismo) '''
''' el atributo tiempo de preparacion y el metodo calcular tiempo preparacion estan encapsulados, es decir que son inalcanzables e invisibles desde el exterior de la clase'''
...
'''guardamos esta informacion de recetas en una base de datos: '''
...
''' ANALISIS DE DATOS Y DECISIONES DE NEGOCIO'''
...
'''ARMAMOS UN DATAFRAME EN BASE A LOS MESES Y LAS CANTIDADES DE TORTAS QUE SE VENDIERON POR MES.
...
OBSERVAMOS EN UN GRAFICOS DE BARRAS ESTA INFORMACION.
...
# @app.route('/tortas/<torta>', methods=['GET']) CONSULTANDO UN UNICO REGISTRO
# def tortasGetUnico(torta):
#     for t in tortas:
#         if t['Nro pedido'] == torta:
#             return jsonify({"torta":tortas, "busqueda":torta, "status":"OK"})
#     return jsonify({"busqueda":torta, 'status': 'NOT FOUND'})

# @app.route('/tortas/<torta>', methods=['PUT']) #ACTUALIZAR UN PEDIDO
# def tortasPut(torta):
#     body = request.json
#     nro_pedido = body['Nro pedido']
...
#     for t in tortas:
#         if t['Nro pedido'] == torta:
#             return jsonify({'torta':{'Nro pedido': nro_pedido, 'Tipo torta': tipo_torta, 'Sabor': sabor, 'Relleno': relleno, 'Tamanio': tamanio,  'Cantidad': cantidad}, 'busqueda':torta, 'status':'ok'})
#     return jsonify({'busqueda':torta, 'status': 'NOT FOUND'})
...
# sentenciaSQL = sentenciaSQL + "fecha TIMESTAMP NOT NULL," #timestamp para que lo ponga en fecha y hora
...
'''primero RUN los create table, despues ingreso los valores, y comento los create'''
...
#
# def comprobar_fecha(text): #que el dato que ingrese este en el formato adecuado
#     try:
...
# clientes = [
#             (nro, nombre, apellido, telefono, orden, precio)
#         ]
...
''' Consultando un unico registro: Chocolate'''

# import sqlite3
#
# conexion = sqlite3.connect("Clientes.db")
# cursor = conexion.cursor()
# sentenciaSQL3 = "SELECT * FROM Materia_Prima WHERE nombre = 'Chocolate'"
# cursor.execute(sentenciaSQL3)
# materiaprima = cursor.fetchone()
# print('Registro recuperado: ', materiaprima) #me muestra en tupla el nombre y stock
# print('Stock de la materia prima: ', materiaprima[1]) #me muestra el stock de la materia prima
# conexion.close()
...
# conexion = sqlite3.connect("Clientes.db")
# cursor = conexion.cursor()
# sentenciaSQL4 = "DELETE FROM Materia_Prima WHERE nombre = 'Huevos'"
# cursor.execute(sentenciaSQL4)
# conexion.commit()
# conexion.close()
...
# conexion = sqlite3.connect("Clientes.db")
# cursor = conexion.cursor()
# sentenciaSQL5 = "UPDATE Materia_Prima SET stock = stock + 100 WHERE nombre = 'Harina'"
# cursor.execute(sentenciaSQL5)
# conexion.commit()
# conexion.close()
...
# r = modo lectura. w = modo escritura. a = modo append

''' EJECUTAR DESDE LA TERMINAL: Automatizacion '''

# import sys
# argv = sys.argv
# nombre = argv[1]
# print('Hola mundo! Bienvenido', nombre) #poner en terminal python TPFINAL.py Josefina
